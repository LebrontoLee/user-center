package com.lee.usercenterbackend.service.impl;
...
import com.lee.usercenterbackend.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
     ...
     * 盐值，混淆密码
     */
    private static final String SALT = "lebronto";
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword, String planetCode) {
        ...
        if(planetCode.length()>5){
            throw new BusinessException(ErrorCode.PARMS_ERROR,"星球账号过长");
        }
        // 校验账户不能包含特殊字符
        ...
        if(!userPassword.equals(checkPassword)){
            throw new BusinessException(ErrorCode.PARMS_ERROR,"密码和校验密码不一致");
        }
        // 账户不能重复
        ...
        if(count>0){
            throw new BusinessException(ErrorCode.PARMS_ERROR,"星球编号重复");
        }
        // 2、加密
        ...
        if(!saveResult){
            throw new BusinessException(ErrorCode.PARMS_ERROR,"保存用户失败");
        }
        return user.getId();
    }
    ...
    public User userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        if(user==null)
        {
            throw new BusinessException(ErrorCode.PARMS_ERROR,"用户不存在");
        }
        // 3、用户脱敏
